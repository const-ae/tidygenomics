...
The most dramatic impact on programming in R lately was the developement of the [tidyverse](http://tidyverse.org/) by Hadley Wickham et al.
which, combined with the ingenious `%>%` from magrittr, provides a uniform philsophie for handling data.
...
- `genome_cluster`
- `genome_complement`
- `genome_join` _Implemented by the fuzzyjoin package_
...
Subtract one data frame from the other. This can be used to split the x data frame into smaller areas.
...
Joins 2 data frames based on their genomic location. If no exact overlap is found the closest element is used.
...
Add a new column with the cluster id, if 2 intervals are overlapping or within `max_distance`.
